...
class Validator:
    ...
    def lint_markdown_files(self):
        ...
        try:
            ...
            initial_issue_count = (
                len(initial_issues.splitlines()) if initial_issues else 0
            )

            if (
                scan_result.returncode != 0
                and "command not found" in scan_result.stderr.lower()
            ):
                print("[ERROR] 'pymarkdown' command not found.")
                print(
                    "Please ensure it is installed and in your PATH: pip install pymarkdown-linter"
                )
                return False
            ...
            remaining_issue_count = (
                len(remaining_issues.splitlines()) if remaining_issues else 0
            )
